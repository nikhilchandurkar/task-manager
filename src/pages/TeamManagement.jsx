...
function TeamManagementContent() {
  ...
  // Fetch Teams from Firestore (try multiple approaches)
  useEffect(() => {
    if (!currentUser?.uid) {
      setLoading(false);
      return;
    ...
    const fetchTeams = async () => {
      try {
        ...
        // Try another approach - get all teams and check membership client-side
        ...
        teamsSnapshot.forEach(doc => {
          ...
          // Check different possible membership structures
          let isMember = false;
          ...
          // Check if the user is in the memberIds array
          if (Array.isArray(teamData.memberIds) && teamData.memberIds.includes(currentUser.uid)) {
            isMember = true;
          }
          
          // Check if the user is in a members object/map
